...
class AbTestTest < ActionController::TestCase
  ...
  def test_returns_different_alternatives_for_each_participant
    ...
    # the first alternative is always returned when disconnected
    assert_equal %w{foo}, alts.uniq.sort
    assert_equal 1000, alts.select { |a| a == "foo" }.size
  end
  ...
  def test_ab_test_chooses_in_render
    ...
    assert_equal %w{false}, responses.uniq.sort
  end
  ...
  def test_ab_test_with_capture
    ...
    assert_equal %w{false}, responses.map(&:strip).uniq.sort
  end
  ...
  def test_scoring_with_different_probability
    ...
    assert_nil experiment(:abcd).score(90).choice
    assert_nil experiment(:abcd).score(95).choice
